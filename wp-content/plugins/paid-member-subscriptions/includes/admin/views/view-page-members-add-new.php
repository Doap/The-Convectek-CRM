...
<div class="wrap">
    ...
    <form id="pms-form-add-new-member" class="pms-form" method="POST">
        ...
        <div class="pms-form-field-wrapper">
            ...
            <select id="pms-member-username" name="pms-member-username" class="widefat">
                <option value=""><?php echo __( 'Select...', 'paid-member-subscriptions' ); ?></option>
                <?php
                    $users = pms_get_users_non_members();

                    foreach( $users as $user ) {
                        echo '<option ' . ( isset( $this->request_data['pms-member-user-id'] ) ? selected( $this->request_data['pms-member-user-id'], $user->ID, false ) : '' ) . ' value="' . $user->ID . '">' . $user->data->user_login . '</option>';
                    }
                ?>
            </select>
            <input type="hidden" id="pms-member-user-id" name="pms-member-user-id" class="widefat" value="<?php echo ( isset( $this->request_data['pms-member-user-id'] ) ? $this->request_data['pms-member-user-id'] : 0 ); ?>" />
        ...
        <?php do_action( 'pms_memeber_add_new_form_field' ); ?>
