        ...
        if( $pms_add_ons === false ) {
            ...
            foreach( $pms_add_ons as $key => $pms_add_on ) {
                ...
                if( !( $pms_add_on['paid'] && $pms_add_on_exists ) )
                    ...
                    echo '<div class="pms-add-on-serial-number-wrapper' . $serial_status_class . '"><input class="pms-add-on-serial-number'.$serial_status_class.'" type="text" value ="'. $serial_number .'" data-unique-name="'. $pms_add_on['unique_name'] .'" data-slug="'. $pms_add_on['slug'] .'" name="' . $pms_add_on['slug'] . '_serial_number" placeholder="' . __("Serial Number", 'paid-member-subscriptions') . '"/><span class="status-dot"></span><a href="#" class="button save-serial">' . __('Save', 'paid-member-subscriptions') . '</a></div>';
                }

                echo '<p class="pms-add-on-description">' . $pms_add_on['description'] . '</p>';
                    ...
                    if (in_array(strtolower($version), $pms_add_on['product_version_type'])) {
                        ...
                        if ($pms_add_on_exists) {
                            ...
                            if (!$pms_add_on_is_active) {
                                echo '<a class="pms-add-on-activate right button button-secondary" href="' . $pms_add_on['plugin_file'] . '" data-nonce="' . $ajax_nonce . '">' . __('Activate', 'paid-member-subscriptions') . '</a>';

                                // If add-on is network activated don't allow deactivation
                            } elseif (!$pms_add_on_is_network_active) {
                                echo '<a class="pms-add-on-deactivate right button button-secondary" href="' . $pms_add_on['plugin_file'] . '" data-nonce="' . $ajax_nonce . '">' . __('Deactivate', 'paid-member-subscriptions') . '</a>';
                            }
                            ...
                            echo '<a target="_blank" class="right button ' . $pms_paid_link_class . '" href="' . $pms_add_on['download_url'] . $pms_paid_href_utm_text . '" data-add-on-slug="paid-member-subscriptions-' . $pms_add_on['slug'] . '" data-add-on-name="' . $pms_add_on['name'] . '" data-nonce="' . $ajax_nonce . '">' . $pms_paid_link_text . '</a>';
