...
Class PMS_Emails{
    ...
    public function __construct() {
        add_action( 'pms_register_form_after_create_user', array( $this, 'send_registration_email' ) );

        add_action( 'pms_member_update_subscription', array( $this, 'send_emails' ), 10, 6 );
        add_action( 'pms_member_add_subscription', array( $this, 'send_emails' ), 10, 6 );

        /* change email headers */
        add_filter('wp_mail_from_name', array( $this, 'pms_email_website_name' ), 20, 1);
        add_filter('wp_mail_from', array( $this, 'pms_email_website_email' ), 20, 1);
     ...
     */
    function send_emails( $update_result, $user_id, $subscription_plan_id, $start_date, $expiration_date, $status ){
        if( $update_result ){
            if( $status == 'active' ){
                PMS_Emails::pms_mail( 'activate', $user_id, $subscription_plan_id );
            }
            else if( $status == 'canceled' ){
                PMS_Emails::pms_mail( 'cancel', $user_id, $subscription_plan_id );
            }
            else if( $status == 'expired' ){
                PMS_Emails::pms_mail( 'expired', $user_id, $subscription_plan_id );
            }
        }
    }

    function send_registration_email( $user_data ){
        ...
        if( !( defined( 'PROFILE_BUILDER' ) && PROFILE_BUILDER == 'Profile Builder Pro' && !empty( $wppb_addonOptions['wppb_emailCustomizer'] ) && $wppb_addonOptions['wppb_emailCustomizer'] == 'show' ) )
            PMS_Emails::pms_mail( 'register', $user_data['user_id'], $user_data['subscriptions'] );
    }
     ...
     */
    static function pms_mail( $action, $user_id, $subscription_plan_id ){

        $user_info = get_userdata( $user_id );

        $email_default_subjects = PMS_Emails::get_email_subjects();
        $email_default_content = PMS_Emails::get_email_content();
        ...
        $email_subject = PMS_Merge_Tags::pms_process_merge_tags( $email_subject, $user_info, $subscription_plan_id );
        $email_content = PMS_Merge_Tags::pms_process_merge_tags( $email_content, $user_info, $subscription_plan_id );
     ...
     */
    static function get_email_headers(){
        $email_headers = array(
     ...
     */
    static function get_email_subjects(){
     ...
     */
    static function get_email_content(){
        $email_content = array(
    ...
    function pms_email_website_name( $site_name ){
        $pms_settings = get_option( 'pms_settings' );
    ...
    function pms_email_website_email( $sender_email ){
        $pms_settings = get_option( 'pms_settings' );
