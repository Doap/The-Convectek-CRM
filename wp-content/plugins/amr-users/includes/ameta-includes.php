...
global $amain;
	...
	$base = remove_query_arg (array('refresh','listpage','rows_per_page'));
	...
	if (!empty($_REQUEST['rows_per_page'])) {
		$rpp = (int) $_REQUEST['rows_per_page'];
		if (!($rpp === $amain['rows_per_page']))
			$base = add_query_arg('rows_per_page',$rpp,$base);  // int will force to a number
	}	
//	if (!empty($_SERVER['QUERY_STRING']) ) $format = '&listpage=%#%'; // ?page=%#% : %#% is replaced by the page number
//	else $format = '?listpage=%#%';
	return ($base); //201508 - causing double escaping / encoding and admin errors so remove the escurl
}
/* -----------------------------------------------------------------------------------*/
...
function amr_is_bulk_request ($type) {
	if (((isset($_POST['dobulk']))
	 or (isset($_POST['dobulk2']) ) )
	and 
	((!empty($_POST['action']) and ($_POST['action'] == $type))
	or
	(!empty($_POST['action2']) and ($_POST['action2'] == $type)	))
	) {
...
function amr_redirect_if_delete_requested () { 
	if (amr_is_bulk_request ('delete'))	{
		...
		if (isset($_POST['users'])) {
			//echo 'Redirecting to delete screen  <br />'; //***
			wp_redirect(
			add_query_arg(array(
			'users'=>$_POST['users'] , 
			'action'=>'delete'
...
add_action('admin_init','amr_redirect_if_delete_requested');
