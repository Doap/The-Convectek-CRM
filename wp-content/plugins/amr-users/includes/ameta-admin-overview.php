...
function amr_echo_setting_html($setting, $i, $status=true) { // 20150820
global $amain;
	//$i=list number, status is input field status
		echo '<td align="center"><input type="checkbox" id="'.$setting['name']
			.$i.'" name="'.$setting['name'].'['. $i .']" value="1" '.$status;
		if (!empty($amain[$setting['name']][$i])) echo 'checked="Checked"'; 
		echo '/></td>';
}
...
//
		...
		if (!empty($addon_settings)) { //20150820
			foreach ($addon_settings as $setting) {
				amr_echo_setting_html($setting, $i, $status);
			}
		}
		
//	later all checkbox settings should use above code
